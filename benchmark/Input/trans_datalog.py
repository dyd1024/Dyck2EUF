...
for analysisType in analysis_Type:
    ...
    for j in range(2):
        ...
        for bm in benchmarks:
            ...
            with open(f'./SPG_{analysisType}/0/{bm}.spg', 'r') as f0:
                 for line in f0:
                    #  line = "17300->1780425009[label=\"op--1173\"]"
                     ...
                     if(len(matchs) != 3):
                         ...
                         m0 = matchs[0]
                         m1 = matchs[1]
                         var_declar.add(matchs[0])
                         var_declar.add(matchs[1])

                         temp = max(temp, int(m0), int(m1))

                         if matchs[2] == "-1":
                             result_bdd.append(f'eq({m0}, {m1}).\n')
                             result_dl.append(f'eq({m0}, {m1}).\n')
                         else:
                             result_bdd.append(f'fun({m1}, {(matchs[2])}, {m0}).\n')
                             result_dl.append(f'fun({m1}, {(matchs[2])}, {m0}).\n')
                 i = 0
                 while i < seq_length:
        ...
        #dot2bdd
            with open(f'./bdd_{analysisType}/{seq_length}/{bm}.datalog', 'w') as f8:
                ...
                for a in query:
                    ...
                    f8.write(f'q{que_1}(x, y) :- eq(x, y), x = {a[0]}, y = {a[1]}.\n')
                    que_1 = que_1+1
        #dot2dl
            with open(f'./dl_{analysisType}/{seq_length}/{bm}.dl', 'w') as f9:
                ...
                for a in query:
                    f9.write(f"iseq(x, y) :- eq(x, y), x={a[0]}, y={a[1]}.\n")
end_time = time.time()
